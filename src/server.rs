...
fn ensure_connected_devices_development_config(path: &Path) -> Result<bool> {
    ...
    let (without_legacy_toggle, removed_legacy_toggle) =
        remove_environment_name(&contents, "MISTY_CONNECTED_DEVICES_ENABLED");
    contents = without_legacy_toggle;
    ...
    if additions.is_empty() && !removed_legacy_toggle {
        return Ok(false);
    }
...
fn remove_environment_name(contents: &str, name: &str) -> (String, bool) {
    ...
    if ended_with_newline && !updated.is_empty() {
        updated.push('\n');
    }
    (updated, removed)
}
...
mod tests {
    ...
    fn environment_values_find_only_exact_active_keys() {
        ...
        assert_eq!(
            environment_values(
                "JOURNAL_COLLAB_ROOM_SALT=\"quoted\"\n",
    ...
    #[test]
    fn development_server_removes_the_legacy_connected_devices_toggle() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join(".env.dev");
        fs::write(&path, "MISTY_CONNECTED_DEVICES_ENABLED=false\n").unwrap();

        assert!(ensure_connected_devices_development_config(&path).unwrap());
        let contents = fs::read_to_string(&path).unwrap();
        assert!(!contents.contains("MISTY_CONNECTED_DEVICES_ENABLED"));
        assert_eq!(
            environment_values(&contents, DEVICE_TICKET_PRIVATE_KEY).len(),
            1
        );
        assert_eq!(
            environment_values(&contents, DEVICE_PAIRING_PEPPER).len(),
            1
        );
    }
